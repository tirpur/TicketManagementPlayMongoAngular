...
import models.{Category, Assignee}
...
class Categories @Inject() (
  ...
  val collection = db[BSONCollection]("categories")

  /** list all tickets */
  def index = Action.async { implicit request =>
    //    Async {
    val cursor = collection.find(
        BSONDocument(), BSONDocument()).cursor[Category]() // get all the fields of all the tickets
